...
5. Highlights (Canvas snippet): buy Symbol orange if LastDay% or Today% > 3; sell orange if LastDay% or Today% < -3; colour **MLBuy** / **MLSell** tokens in Why via Text `#C6F6D5` / `#FEB2B2`; **News catalyst** Positive `#C6F6D5` / Negative `#FEB2B2` / Mixed `#FEFCBF`; **Sentiment** green when Bullish + positive news, red when Bearish + negative news. **Momentum keyword chips** in Why: green `#C6F6D5` on UpStairs when Today% > 2 or UpPostLunchConsolidation when Today% > 3; red `#FEB2B2` on DownStairs when Today% < -2 or DownPostLunchConsolidation when Today% < -3 (keyword only — not full row).
