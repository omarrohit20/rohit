# Priority Picks → Chat + Cursor Canvas (required)

Always present the **same Priority picks board in both places**:

1. **Chat** — full markdown Priority table (readable inline; colours may not
   render because chat strips HTML `style`)
...
1. Write/update the `.canvas.tsx` with coloured highlights.
2. In the chat reply, lead with the **full Priority markdown table** (all
   columns) **and** a markdown link to the Canvas.
3. Continue with exec summary / news / avoid / disclaimer in chat.
...
```markdown
## Priority Picks
[Coloured board](…/canvases/intraday-priority-picks.canvas.tsx)
