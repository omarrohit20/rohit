...
6. Highlights (Canvas snippet): green Symbol if (BreakHighYear|BreakHighYear2) + ReversalLow + Today% in [−1.3, 1.3]; buy Symbol orange if LastDay% or Today% **> 3**; sell orange if **< -3**; colour the **MLBuy** / **MLSell** tokens in Why via Text `#C6F6D5` / `#FEB2B2` — not the scrip; **News catalyst** Positive `#C6F6D5` / Negative `#FEB2B2` / Mixed `#FEFCBF` (`news-extension.md`).
