...
def futures_data(scrip):
    stock_fut = get_history(symbol=scrip,
                            ...
                            futures=True,
                            expiry_date=get_expiry_date(datetime.date.today().year, datetime.date.today().month + 0))
    return stock_fut
