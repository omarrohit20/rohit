...
with col2:
    df = rb.getdf('morning-volume-breakout-buy')
    rb.render(st, df, 'LastDayMarketLT-1:OrDowntrend : todayUpGT0.5', color='G', height=150)
with col3:
    df = rb.getdf('morning-volume-breakout-sell')
    filtered_df = df
    try:
...
with col4:
    df = rb.getdf('morning-volume-breakout-sell')
    rb.render(st, df, 'LastDayMarketGT1:OrUptrend : todayDownLT-0.5', color='R', height=150)
