...
def _filter_scan_news(df, sentiment):
    """Keep rows whose scrip_news overall_sentiment is Bullish or Bearish."""
    if df is None or getattr(df, "empty", True) or "scrip" not in getattr(df, "columns", []):
        return df if df is not None else pd.DataFrame()
    allowed = rb.news_sentiment_scrips(sentiment)
    if not allowed:
        return df.iloc[0:0]
...
def main():
    ...
    #                        page_title="Dashboard0",
    #                        initial_sidebar_state="expanded",)
    # except Exception:
    #     pass

    # main title
    st.title('chartlink-0 : 9:20 - 09:40 Morning')
    ...
    with col1:
        ...
        try:
            ...
            print("")
        rb.render(st, filtered_df, 'RISKY:LDayMarketUpTodayUp(+)', height=150, color='LG')
    with col2:
        df = rb.getdf('morning-volume-breakout-buy')
        filtered_df = df
        try:
            filtered_df = df[
                (
                    ...
                    (df['PCT_day_change_pre2'] < 0) &
                    (   
                        (df['PCT_change'] < -1.2) | (df['PCT_day_change'] < -1.2) | (df['PCT_day_change_pre1'] < -1.2)
                    )
                    &
                    (   
                        (df['PCT_day_change'] < -2) | 
            ...
            print("")
        rb.render(st, filtered_df, 'LastDayMarketLT-1:OrDowntrend : todayUpGT0.5(-)', height=150, color='LG')
    with col3:
        df = rb.getdf('morning-volume-breakout-sell')
        filtered_df = df
        try:
            ...
            print("")
        rb.render(st, filtered_df, 'RISKY:LDayMarketDownTodayDown(-)', height=150, color='LG')
    with col4:
        df = rb.getdf('morning-volume-breakout-sell')
        filtered_df = df
        try:
    ...
    with col1:
        ...
        try:
            ...
            print("")
        rb.render(st, filtered_df, 'LastDayDownTodayUp:buyAfterSettledA10(-)', color='LG')
    with col2:
        df = rb.getdf('morning-volume-breakout-buy')
        filtered_df = df
        try:
            ...
            print("")
        rb.render(st, filtered_df, 'LastDayMarketLT-1 : todayUpGT0.5(-)', color='LG')
    with col3:
        df = rb.getdf('morning-volume-breakout-sell')
        filtered_df = df
        try:
            ...
            print("")
        rb.render(st, filtered_df, 'LastDayUpTodayDown:sellAfterSettledA10(+)', color='LG')
    with col4:
        df = rb.getdf('morning-volume-breakout-sell')
        filtered_df = df
        try:
            ...
            print("")
        rb.render(st, filtered_df, 'LastDayMarketGT1 : todayDownLT-0.5(+)', color='LG')
    ...
    with col2:
        ...
        try:
            ...
            print("")
        rb.render(st, filtered_df, 'Sell Near Month3Low', color='LG')
    ...
    with col1:
        ...
        if len(filtered_df) < 50:
            ...
            rb.render(st, empty_df, 'MorningDown:UpAfterDown', color='LG')
    with col3:
        df = rb.getdf('morning-volume-breakout-sell')
        expected_columns = list(set(df.columns))
        empty_df = pd.DataFrame(columns=expected_columns)
    ...
    with col3:
        ...
        try:
            ...
            print("")
        if len(df) > 5:
            rb.render(st, filtered_df, 'MLBuy', color='G')
        else:
            rb.render(st, empty_df, 'MLBuy', color='G')
    with col4:
        df = rb.getdf('morning-volume-breakout-buy')
        empty_df = pd.DataFrame(columns=expected_columns)
        filtered_df = df
    ...
    with col7:
        ...
        try:
            ...
            print("")
        if len(df) > 5:
            rb.render(st, filtered_df, 'MLSell', color='R')
        else:
            rb.render(st, empty_df, 'MLSell', color='R')
    with col8:
        df = rb.getdf('morning-volume-breakout-sell')
        empty_df = pd.DataFrame(columns=expected_columns)
        filtered_df = df
    ...
    with col1:
        ...
        try:
            ...
            print("")
        if len(filtered_df) < 20:
            rb.render(st, filtered_df, 'Volume - BreakHigh', color='LG')
        else:
            rb.render(st, empty_df, 'Volume - BreakHigh', color='LG')
    with col2:
        df = rb.getdf('morning-volume-breakout-buy')
        expected_columns = list(set(df.columns))
        empty_df = pd.DataFrame(columns=expected_columns)
        ...
        try:
            ...
            print("")
        if len(filtered_df) < 20:
            rb.render(st, filtered_df, 'Volume - ReversalLow', color='LG')
        else:
            rb.render(st, empty_df, 'Volume - ReversalLow', color='LG')
    with col3:
        df = rb.getdf('morning-volume-breakout-sell')
        expected_columns = list(set(df.columns))
        empty_df = pd.DataFrame(columns=expected_columns)
        ...
        try:
            ...
            print("")
        if len(filtered_df) < 20:
            rb.render(st, filtered_df, 'Volume - BreakLow', color='LG')
        else:
            rb.render(st, empty_df, 'Volume - BreakLow', color='LG')
    with col4:
        df = rb.getdf('morning-volume-breakout-sell')
        expected_columns = list(set(df.columns))
        empty_df = pd.DataFrame(columns=expected_columns)
    ...
    with col1:
        ...
        try:
            ...
            print("")
        rb.render(st, filtered_df, 'year5HighChangeLT-30 + BreakHighMonth3', color='LG')
    with col2:
        df = rb.getdf('morning-volume-breakout-buy')
        filtered_df = df
        try:
            filtered_df = df[
                ...
                (df['forecast_day_PCT10_change'] < 8) &
                ((df['forecast_day_PCT10_change'] > 1) | (df['PCT_day_change'] < -1)) &
                (df['PCT_day_change'] < 1.5) &
                (df['PCT_day_change_pre1'] > -0.5) &
                (df['month3HighChange'] < -5) &
                (df['week2HighChange'] < -2) &
