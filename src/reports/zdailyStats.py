...
def main():
    ...
    with col4:
        ...
        try:
            filtered_df = df[
                ((df['index'].str.contains("futures")) | 
                    ((df['PCT_day_change'] > 2) & ((df['lowTail'] >= 2) | (df['month3HighChange'] > 3)))) &
                ((df['forecast_day_PCT10_change'] > 7) | (df['PCT_day_change'] > 1)) &
                (df['PCT_day_change'] > -1) &
                ((df['PCT_change'] < -2) | (df['PCT_day_change_pre1'] < -2) | (df['PCT_day_change_pre2'] < -2)) &
        ...
        try:
            filtered_df = df[
                (
                    ...
                    (df['index'].str.contains("futures")) &
                    (df['forecast_day_PCT10_change'] > 10) &
                    (df['intradaytech'].str.contains("morningUp") )
                )
                |
                (
                    (df['index'].str.contains("futures")) &
            ...
            print("")
        rb.render(st, filtered_df, 'UpTrend-1', column_conf=rb.column_config_result, column_order=rb.column_order_result, renderml=True, color='LG')
    ...
    with col4:
        ...
        try:
            filtered_df = df[
                ((df['index'].str.contains("futures")) | 
                    ((df['PCT_day_change'] < -2) & ((df['highTail'] >= 2) | (df['month3LowChange'] < -3)))) &
                ((df['forecast_day_PCT10_change'] < -7) | (df['PCT_day_change'] < -1)) &
                (df['PCT_day_change'] < 1) &
                ((df['PCT_change'] > 2) | (df['PCT_day_change_pre1'] > 2) | (df['PCT_day_change_pre2'] > 2)) &
                ((df['PCT_day_change'] < -1) | (df['PCT_day_change_pre1'] < -1) | (df['PCT_day_change_pre2'] < -1)) &
        ...
        try:
            filtered_df = df[
                (
                    ...
                    (df['index'].str.contains("futures")) &
                    (df['forecast_day_PCT10_change'] < -10) &
                    (df['intradaytech'].str.contains("morningDown") )
                )
                |
                (
                    (df['index'].str.contains("futures")) &
            ...
            print("")
        rb.render(st, filtered_df, 'DownTrend-1', column_conf=rb.column_config_result, column_order=rb.column_order_result, renderml=True, color='LG')
