...
from Algorithms.regression_helpers import load_dataset, addFeatures, addFeaturesVolChange, \
    ...
    mergeDataframes, count_missing, applyTimeLag, performClassification, performClassificationTest   
...
directory = '../../output/classifierTest' + '/' + time.strftime("%d%m%y-%H%M%S")
logname = '../../output/classifierTest' + '/mllog' + time.strftime("%d%m%y-%H%M%S")
...
def regression_ta_data(scrip):
    ...
    if bagging:
        regressionResult.extend(performClassification(dfp, 0.98, scrip, directory, forecast_out, SVC(C=1.0, cache_size=500, class_weight=None, coef0=0.0,
    ...
    if kNeighbours:
        ...
        regressionResult.extend(performClassificationTest(get_data_frame(df, 'kNeighbours'), 0.98, scrip, directory, forecast_out))
    else:
        regressionResult.extend([0,0])
