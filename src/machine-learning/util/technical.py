...
# def ta_lib_data(scrip, TEST=False):
#     if TEST:
...
def ta_lib_data_df(scrip, dfp, db_store=False, TEST=False):
    try:
        if((db_store==True) and (TEST==False)):
...
#             return
        ...
        technical_indicators={}
        technical_indicators['dataset_code'] = scrip
        technical_indicators['end_date'] = end_date
        technical_indicators['change'] = change
        technical_indicators['pChange'] = pchange
        technical_indicators['trend'] = trend
        technical_indicators['yearHigh'] = yearHigh
...
#         volatility_indicators['TRANGE'] = TRANGE(historicalInputs).tolist()[::-1]
        ...
        if technical_indicators['BuyIndicatorsCount'] > 0:
            log.info('%s Buy: %s', scrip, technical_indicators['BuyIndicators'])
        if technical_indicators['SellIndicatorsCount'] > 0:
            log.info('%s Sell: %s', scrip, technical_indicators['SellIndicators']) 
