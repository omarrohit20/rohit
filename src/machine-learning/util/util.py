...
def buy_pattern_without_mlalgo(regression_data, regressionResult, ws_buyPattern2, ws_sellPattern2):
    ...
    if(regression_data['buyIndia_avg'] > 0.9 and high_tail_pct(regression_data) < 1.5
    ...
    if(regression_data['close'] > 50
        ...
        if(regression_data['SMA25'] > 0
            ...
            if(regression_data['PCT_day_change_pre1'] > 1
                ...
                add_in_csv(regression_data, regressionResult, ws, '##ALL:MLBuyRisingMA-Risky-0')
            elif(is_algo_sell(regression_data)):
                add_in_csv(regression_data, regressionResult, ws, '##ALL:MLSellRisingMA-Risky-0')
            ...
            if(-4 < regression_data['PCT_day_change'] < -1
                ...
                add_in_csv(regression_data, regressionResult, ws, '##ALL:MLSellRisingMA-Risky-1')
        elif(regression_data['SMA4_2daysBack'] > 0
            and regression_data['SMA9'] > 1
            ...
            if(is_algo_buy(regression_data)):
                add_in_csv(regression_data, regressionResult, ws, '##ALL:MLBuyRisingMA-Risky-2')
            elif(is_algo_sell(regression_data)):
                add_in_csv(regression_data, regressionResult, ws, '##ALL:MLSellRisingMA-Risky-2')
            ...
            if (regression_data['month3LowChange'] > 20
                ...
                if(is_algo_buy(regression_data)):
                    add_in_csv(regression_data, regressionResult, ws, '##ALL:MLbuyRisingMAuptrend-Risky')
                elif(is_algo_sell(regression_data)):
                    add_in_csv(regression_data, regressionResult, ws, '##ALL:MLsellRisingMAuptrend-Risky')
            ...
            if((((regression_data['PCT_change'] + regression_data['PCT_change_pre1'])  < -4
                ...
                if(is_algo_buy(regression_data)):
                    add_in_csv(regression_data, regressionResult, ws, '##ALL:MLbuyRisingMA-down-Risky')
                elif(is_algo_sell(regression_data)):
                    add_in_csv(regression_data, regressionResult, ws, '##ALL:MLsellRisingMA-down-Risky')
        ...
        if(0 < regression_data['forecast_day_PCT2_change'] < regression_data['forecast_day_PCT3_change'] < regression_data['forecast_day_PCT4_change'] < regression_data['forecast_day_PCT5_change']
            ...
            if('SPINNINGTOP' in regression_data['sellIndia']
                ...
                if(regression_data['forecast_day_PCT_change'] < 0
                    ...
                    add_in_csv(regression_data, regressionResult, ws, None)
                elif(('P@' or 'M@') not in regression_data['buyIndia']):
                    add_in_csv(regression_data, regressionResult, ws, '##ALL(Test):sellSuperTrend-1')
        return True
    return False
...
#                 add_in_csv(regression_data, regressionResult, ws, '##ALL:sellMonth6High-InMinus')
        ...
        if((-0.5 < regression_data['PCT_change'] < 0.5)
            ...
            if(abs_month3High_less_than_month3Low(regression_data)
                and regression_data['month3HighChange'] > -7
                and regression_data['PCT_day_change'] < 0
                ...
                add_in_csv(regression_data, regressionResult, ws, '##ALL:MaySellCheckChart-(Risky)')
...
#             add_in_csv(regression_data, regressionResult, ws, "##Common:buyHighTailLessThan0.3-2(checkHillUp)")
    ...
    if((mlpValue > 0) 
        ...
        if(regression_data['SMA9'] > 1):
            add_in_csv(regression_data, regressionResult, ws, '##Common:buyNotM3HighLow(SMA9GT1)')
        elif(regression_data['SMA25'] > 0):
            add_in_csv(regression_data, regressionResult, ws, '##Common:buyNotM3HighLow(SMA25GT0)')  
        else:
            add_in_csv(regression_data, regressionResult, ws, '##UPTREND:buyNotM3HighLow')
...
def sell_pattern_without_mlalgo(regression_data, regressionResult, ws_buyPattern2, ws_sellPattern2):
    ...
    if(regression_data['sellIndia_avg'] < -0.9 and low_tail_pct(regression_data) < 1.5
        and regression_data['SMA9'] < -1
        ):
...
#         return True
    ...
    if(-2 > regression_data['month3HighChange'] > -10):
...
#                 add_in_csv(regression_data, regressionResult, ws, '##ALL:buyMonth3Low-InPlus')
        ...
        if((-0.5 < regression_data['PCT_change'] < 0.5)
            ...
            if(abs_month3High_more_than_month3Low(regression_data)
                and regression_data['month3LowChange'] < 7
                and regression_data['PCT_day_change'] > 0
                ...
                add_in_csv(regression_data, regressionResult, ws, '##ALL:MayBuyCheckChart-(Risky)') 
...
def sell_all_common(regression_data, regressionResult, reg, ws):
    ...
    if((mlpValue < 0) 
        ...
        if(regression_data['SMA9'] < -1):
            add_in_csv(regression_data, regressionResult, ws, '##Common:sellNotM3HighLow(SMA9LT-1)') 
        elif(regression_data['SMA25'] < 0):
            add_in_csv(regression_data, regressionResult, ws, '##Common:sellNotM3HighLow(SMA25LT0)') 
        else:
            add_in_csv(regression_data, regressionResult, ws, '##DOWNTREND:sellNotM3HighLow') 
    ...
    if(is_algo_sell(regression_data)
        ...
        if(('nearYear2High' in regression_data['filter3'])):
            add_in_csv(regression_data, regressionResult, ws, None)
        if(('nearYearHigh' in regression_data['filter3'])):
            add_in_csv(regression_data, regressionResult, ws, '##DOWNTREND:sellYearHigh')
        if(('nearMonth6High' in regression_data['filter3'])):
            add_in_csv(regression_data, regressionResult, ws, '##DOWNTREND:sellMonth6High')
        if('yearLowBreak' in regression_data['filter3']):
