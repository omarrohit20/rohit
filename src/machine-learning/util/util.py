...
def add_in_csv(regression_data, regressionResult, ws=None, filter=None, filter1=None, filter2=None, filter3=None, filter4=None, filter5=None, filter6=None):
    if(TEST != True):
        if(is_algo_buy(regression_data, True) and (filter is not None)):
            if '[MLBuy]' not in regression_data['filter']:
                regression_data['filter'] = regression_data['filter'] + '[MLBuy]' + ':'
        if(is_algo_sell(regression_data, True) and (filter is not None)):
...
def is_algo_buy_classifier(regression_data, not_from_rule=False):
...
def is_algo_sell_classifier(regression_data, not_from_rule=False):
    if((regression_data['mlpValue_cla'] <= 0) and (regression_data['kNeighboursValue_cla'] <= 0)
        and ((regression_data['mlpValue_cla_other'] <= 0) and (regression_data['kNeighboursValue_cla_other'] <= 0))
        and ((regression_data['mlpValue_cla_other'] <= -1) or (regression_data['kNeighboursValue_cla_other'] <= -1))
#         and ((regression_data['PCT_day_change'] < -1 
...
#             add_in_csv(regression_data, regressionResult, None, 'MayBuy-upTrend', None)
    ...
    if(-5 < regression_data['PCT_day_change'] < -1.5
        ...
        and 4 > low_tail_pct(regression_data) >= 1.49 and high_tail_pct(regression_data) <= 0.8):
            add_in_csv(regression_data, regressionResult, None, None, '(May-1-BuyCheckChart-Risky)', None)
        ...
        and 3 > low_tail_pct(regression_data) >= 0.99 and high_tail_pct(regression_data) <= 0.6):
            add_in_csv(regression_data, regressionResult, None, None, '(May-2-BuyCheckChart-Risky)', None)
    ...
    if(1.5 < regression_data['PCT_day_change'] < 5
        ...
        and 4 > high_tail_pct(regression_data) >= 1.49 and low_tail_pct(regression_data) <= 0.8):
            add_in_csv(regression_data, regressionResult, None, None, '(May-1-SellCheckChart-Risky)', None)   
        ...
        and 3 > high_tail_pct(regression_data) >= 0.99 and low_tail_pct(regression_data) <= 0.6):
            add_in_csv(regression_data, regressionResult, None, None, '(May-2-SellCheckChart-Risky)', None)
    ...
    if(regression_data['buyIndia_avg'] > 0.9 and regression_data['buyIndia_count'] > 1
        ...
        if(regression_data['yearLowChange'] > 10 and regression_data['buyIndia_count'] > 1):
            if(regression_data['buyIndia_avg'] > 1.5):
                ...
                add_in_csv(regression_data, regressionResult, None, 'buyPattern-GT0.9-SMAGT0')
        ...
        if(regression_data['yearLowChange'] > 10 and regression_data['buyIndia_count'] > 1):
            if(regression_data['buyIndia_avg'] < -1.5):
                ...
                add_in_csv(regression_data, regressionResult, None, 'sellPattern-LT-0.9-SMALT0')
...
def buy_base_line(regression_data, regressionResult, reg, ws):
    ...
    if(mlpValue > 0
        ...
        if(('year2LowReversal(Confirm)' in regression_data['filter3']) and (regression_data['year2HighChange'] < -40)):
            add_in_csv(regression_data, regressionResult, ws, '##Common:UPTREND:buyYear2LowReversal(Confirm)')
        if(('yearLowReversal(Confirm)' in regression_data['filter3']) and (regression_data['yearHighChange'] < -30)):
            add_in_csv(regression_data, regressionResult, ws, '##Common:UPTREND:buyYearLowReversal(Confirm)')
        if(('month6LowReversal(Confirm)' in regression_data['filter3']) and (regression_data['month6HighChange'] < -25)):
            add_in_csv(regression_data, regressionResult, ws, '##Common:UPTREND:buyMonth6LowReversal(Confirm)')  
        if(('nearYear2Low' in regression_data['filter3']) and (regression_data['year2HighChange'] < -40)):
            add_in_csv(regression_data, regressionResult, ws, '##Common:UPTREND:buyYear2Low')
        if(('nearYearLow' in regression_data['filter3']) and (regression_data['yearHighChange'] < -30)):
            add_in_csv(regression_data, regressionResult, ws, '##Common:UPTREND:buyYearLow')
        if(('nearMonth6Low' in regression_data['filter3']) and (regression_data['month6HighChange'] < -25)):
            add_in_csv(regression_data, regressionResult, ws, '##Common:UPTREND:buyMonth6Low')     
...
def buy_all_rule(regression_data, regressionResult, buyIndiaAvg, ws):
    if(is_algo_buy(regression_data, True)
        ...
        if((7.5 < regression_data['month3LowChange'] < 25)
            ...
            add_in_csv(regression_data, regressionResult, None, 'ML:downLastDay-UpTrend')
...
#                 add_in_csv(regression_data, regressionResult, None, 'UPTREND:ML:buy-2(Risky)')
        ...
        if((-2.5 < regression_data['PCT_day_change'] <= -1)
            ...
            if(is_algo_buy(regression_data)
                ...
                add_in_csv(regression_data, regressionResult, ws, "##ML:(check-chart-2-3MidCapCross)MayBuyCheckChart-0")
                return True
...
def buy_all_common(regression_data, regressionResult, reg, ws):
    ...
    if((-2.5 < regression_data['PCT_day_change'] <= -1)
        ...
        if(is_algo_buy(regression_data)
            ...
            add_in_csv(regression_data, regressionResult, ws, "##Common:ML:(check-chart-2-3MidCapCross)MayBuyCheckChart-0")
            return True
        elif(regression_data['PCT_day_change'] <= -0.5):
            add_in_csv(regression_data, regressionResult, ws, None)
            return True
    ...
    if(('MayBuyCheckChart' in regression_data['filter1'])
        ...
        if(is_algo_buy(regression_data)):
            add_in_csv(regression_data, regressionResult, ws, "##Common:ML:(check-chart-2-3MidCapCross)MayBuyCheckChart-1")
            return True
...
def buy_other_indicator(regression_data, regressionResult, reg, ws):
    ...
    buy_base_line(regression_data, regressionResult, reg, ws)
...
#             ):    
#             if('yearHighBreak' in regression_data['filter3']):
#                 add_in_csv(regression_data, regressionResult, ws, '##buyYearHighBreak')
#             if('month6HighBreak' in regression_data['filter3']):
#                 add_in_csv(regression_data, regressionResult, ws, '##buyMonth6HighBreak')
#             if('month3HighBreak' in regression_data['filter3']):
    ...
    if(ema_diff > 0
       ...
       and ema_diff_pre2 < ema_diff_pre1
        ...
        if(1 < regression_data['PCT_day_change'] < 3
           ...
           and 7 > regression_data['SMA4'] > 2
           and 7 > regression_data['SMA9'] > 2
           and 7 > regression_data['SMA4_2daysBack'] > 2
           and 7 > regression_data['SMA9_2daysBack'] > 2
           and 7 > regression_data['SMA25'] > 2
           and regression_data['yearLowChange'] > 20
           and high_tail_pct(regression_data) < 1
           ):
            add_in_csv(regression_data, regressionResult, ws, 'buy-EMA6>EMA14')
            return True
        elif( 0 < regression_data['PCT_day_change'] < 1.5
           and 0 < regression_data['PCT_change'] < 2
           and 5 > regression_data['SMA4'] > 1
           and 5 > regression_data['SMA9'] > 1
           and 5 > regression_data['SMA4_2daysBack'] > 1.5
           and 5 > regression_data['SMA9_2daysBack'] > 1.5
           and 5 > regression_data['SMA25'] > 1
           and regression_data['SMA50'] < 0
           and regression_data['SMA100'] < 0
           and regression_data['SMA200'] < 0
           and ema_diff > 0.2
           #and high_tail_pct(regression_data) < 1
            ...
            add_in_csv(regression_data, regressionResult, ws, 'sell-EMA6>EMA14')
            return True
    if(ema_diff < 0
       and ema_diff_pre1 > 0
       and ema_diff_pre2 > 0
       and ema_diff_pre2 > ema_diff_pre1
        ...
        if(-3 < regression_data['PCT_day_change'] < -1
           ...
           and -7 < regression_data['SMA4'] < 0
           and -7 < regression_data['SMA9'] < 0
           and -7 < regression_data['SMA4_2daysBack'] < -2
           and -5.5 < regression_data['SMA9_2daysBack'] < -2
           and -7 < regression_data['SMA25'] < -2
           and -5 < regression_data['SMA50']
           and low_tail_pct(regression_data) < 1
           ):
            add_in_csv(regression_data, regressionResult, ws, 'sell-EMA6<EMA14')
            return True
        elif(-2 < regression_data['PCT_day_change'] < -1.5
           and -2 < regression_data['PCT_change'] < -0.5
           and -10 < regression_data['SMA4'] < -1
           and -10 < regression_data['SMA9'] < -1
           and -10 < regression_data['SMA4_2daysBack'] < -1.5
           and -10 < regression_data['SMA9_2daysBack'] < -1.5
           and -10 < regression_data['SMA25'] < -1
           and regression_data['year2HighChange'] < -20
           #and regression_data['SMA50'] > 0
           #and regression_data['SMA100'] > 0
           #and regression_data['SMA200'] > 0
           #and low_tail_pct(regression_data) < 1
            ...
            add_in_csv(regression_data, regressionResult, ws, 'sell-1-EMA6<EMA14')
            return True
    ...
    if(regression_data['SMA200'] < regression_data['SMA100'] < regression_data['SMA50'] < regression_data['SMA25']
       ...
       and (regression_data['SMA100'] < -10 or regression_data['SMA200'] < -10)
        ...
        if(-1.5 < regression_data['PCT_day_change'] < -0.5 and -1.5 < regression_data['PCT_change'] < -0.5
            ...
            and -10 < regression_data['month3HighChange'] < -4
            ):
            add_in_csv(regression_data, regressionResult, ws, '(Test):buyDowningMAReversal')
...
#                 )
#            ):
#             add_in_csv(regression_data, regressionResult, ws, '(Test)Sell-EMA6-MT-May-LT-EMA14', None, None, None, None, '(Test)Sell-EMA6-MT-May-LT-EMA14')
#             return True
#         elif(regression_data['PCT_day_change'] > 0
#            and regression_data['SMA4'] > -1
#            and regression_data['SMA25'] > 0
#            ):
#             add_in_csv(regression_data, regressionResult, ws, '(Test)Buy-EMA6-MT-May-LT-EMA14', None, None, None, None, '(Test)Buy-EMA6-MT-May-LT-EMA14')
#             return True
#     if(((regression_data['EMA6'] > regression_data['EMA6_1daysBack'] > regression_data['EMA6_2daysBack'])
#        or (regression_data['EMA14'] > regression_data['EMA14_1daysBack'] > regression_data['EMA14_2daysBack']))
#        and ema_diff_pre2 < ema_diff_pre1 < ema_diff
...
#                 )
#            ):
#             add_in_csv(regression_data, regressionResult, ws, '(Test)Buy-EMA6-LT-May-MT-EMA14', None, None, None, None, '(Test)Buy-EMA6-LT-May-MT-EMA14')
#             return True
#         elif(-4 < regression_data['PCT_day_change'] < 0
#            and abs(regression_data['yearHighChange']) < abs(regression_data['yearLowChange'])
#            and (regression_data['SMA4'] < 1 or regression_data['PCT_day_change'] < -1)
...
#            and regression_data['month6HighChange'] > -20
#            ):
#             add_in_csv(regression_data, regressionResult, ws, '(Test)Sell-EMA6-LT-May-MT-EMA14', None, None, None, None, '(Test)Sell-EMA6-LT-May-MT-EMA14')
#             return True
#         elif(-4 < regression_data['PCT_day_change'] < 0
#            and abs(regression_data['yearHighChange']) > abs(regression_data['yearLowChange'])
#            and ((regression_data['PCT_day_change_pre1'] > 0.5 and regression_data['PCT_day_change_pre2'] > 0.5)
...
#                )
#            ):
#             add_in_csv(regression_data, regressionResult, ws, '(Test)Buy-1-EMA6-LT-May-MT-EMA14', None, None, None, None, '(Test)Buy-1-EMA6-LT-May-MT-EMA14')
#             return True
#         elif(-4 < regression_data['PCT_day_change'] < 0
#            and abs(regression_data['yearHighChange']) > abs(regression_data['yearLowChange'])
#            and ((regression_data['PCT_day_change_pre1'] < -0.5 or regression_data['PCT_day_change_pre2'] < -0.5)
...
#                 )
#            ):
#             add_in_csv(regression_data, regressionResult, ws, '(Test)Sell-1-EMA6-LT-May-MT-EMA14', None, None, None, None, '(Test)Sell-1-EMA6-LT-May-MT-EMA14')
#             return True
    return False    
    ...
    if(regression_data['sellIndia_avg'] < -0.9 and regression_data['sellIndia_count'] > 1
        ...
        if(regression_data['yearHighChange'] < -10 and regression_data['sellIndia_count'] > 1):
            if(regression_data['sellIndia_avg'] < -1.5):
                ...
                add_in_csv(regression_data, regressionResult, None, 'sellPatterns-LT-0.9-SMALT0')   
        ...
        if(regression_data['yearLowChange'] < -10 and regression_data['sellIndia_count'] > 1):
            if(regression_data['sellIndia_avg'] > 1.5):
                ...
                add_in_csv(regression_data, regressionResult, None, 'buyPatterns-GT0.9-SMAGT0')
...
def sell_base_line(regression_data, regressionResult, reg, ws):
    mlpValue, kNeighboursValue = get_reg_or_cla(regression_data, reg)
    mlpValue_other, kNeighboursValue_other = get_reg_or_cla_other(regression_data, reg)
    if((-5 < regression_data['year2LowChange'] < 0 ) 
        and (regression_data['year2HighChange'] < -40)
    ...
    if(is_algo_sell(regression_data)
        ...
        if(regression_data['trend'] == 'down'):
            if(('year2HighReversal(Confirm)' in regression_data['filter3'])):
                add_in_csv(regression_data, regressionResult, ws, '##Common:DOWNTREND:sellYear2HighReversal(Confirm)')
            if(('yearHighReversal(Confirm)' in regression_data['filter3'])):
                add_in_csv(regression_data, regressionResult, ws, '##Common:DOWNTREND:sellYearHighReversal(Confirm)')
            if(('month6HighReversal(Confirm)' in regression_data['filter3'])):
                add_in_csv(regression_data, regressionResult, ws, '##Common:DOWNTREND:sellMonth6HighReversal(Confirm)')
        if('yearLowBreak' in regression_data['filter3']):
            add_in_csv(regression_data, regressionResult, ws, '##Common:DOWNTREND:sellYearLowBreak')
        if('month6LowBreak' in regression_data['filter3']):
            add_in_csv(regression_data, regressionResult, ws, '##Common:DOWNTREND:sellMonth6LowBreak')
...
def sell_all_rule(regression_data, regressionResult, sellIndiaAvg, ws):
    if(is_algo_sell(regression_data, True)
        ...
        if((7.5 < regression_data['month3LowChange'] < 25)
            ...
            add_in_csv(regression_data, regressionResult, None, 'ML:upLastDay-DownTrend')
        if(high_tail_pct(regression_data) < 2.5
           and regression_data['low'] < regression_data['low_pre1']
           ):
            if(regression_data['PCT_day_change'] > 0 and regression_data['PCT_change'] > 0
...
def sell_all_common(regression_data, regressionResult, reg, ws):
    ...
    if((1 < regression_data['PCT_day_change'] < 3) 
        ...
        if(regression_data['PCT_day_change'] >= 1
            ...
            if(is_algo_buy(regression_data)):
                add_in_csv(regression_data, regressionResult, ws, "##Common:ML:(check-chart-2-3MidCapCross)MaySellCheckChart-0")
                return True
            return False
        elif(regression_data['PCT_day_change'] >= 1):
            add_in_csv(regression_data, regressionResult, ws, None)
    ...
    if(('MaySellCheckChart' in regression_data['filter1'])
        ...
        if(is_algo_sell(regression_data)):
            add_in_csv(regression_data, regressionResult, ws, "##Common:ML:(check-chart-2-3MidCapCross)MaySellCheckChart-1")
            return True
        return False
...
def sell_other_indicator(regression_data, regressionResult, reg, ws):
    tail_pct_filter(regression_data, regressionResult)
    sell_base_line(regression_data, regressionResult, reg, ws)
    filterMA(regression_data, regressionResult)
    tail_reversal_filter(regression_data, regressionResult)
    if(regression_data['close'] > 50
        ):
