...
#             ):
        if( 2.9 < regression_data['PCT_day_change'] < 4.1 and 3 < regression_data['PCT_change'] < 4.5
            ...
            add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL:HighUptrend')
            ...
            if(regression_data['PCT_day_change'] < 2.95
                ...
                add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL:HEAVYUPTRENDMARKET:HighUptrend')
            elif(regression_data['PCT_day_change_pre1'] > 0
                or regression_data['PCT_day_change_pre2'] > 0
                or regression_data['PCT_day_change_pre3'] > 0
                ):
                add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL:HEAVYUPTRENDMARKET:HighUptrend-(UPTREND-GLOBALUP)')
        if( 4 < regression_data['PCT_day_change'] < 8 and 3 < regression_data['PCT_change'] < 10
            ...
            if(regression_data['PCT_day_change_pre1'] < 0): 
                add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL(9:30):HighUptrend-2')
            else:
                add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL:HighUptrend-2')
    ...
    if(2.5 < regression_data['PCT_day_change'] < 5 and 2.5 < regression_data['PCT_change'] < 5
        ...
        add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL:UpTrendLast2Day-GlobalUp-GlobalFutureUp-NiftyUp')
...
def sell_all_common_High_Low(regression_data, regressionResult, reg, ws):
    if(((regression_data['forecast_day_PCT_change'] < -0.5 and regression_data['forecast_day_PCT2_change'] < regression_data['forecast_day_PCT_change'])
        ...
        if(-2.9 > regression_data['PCT_day_change'] > -4.1 and -3 > regression_data['PCT_change'] > -4.5
            ...
            add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL:HighDowntrend')
        elif(-2.7 > regression_data['PCT_day_change'] > -5.5
            and -3 > regression_data['PCT_change'] 
            and regression_data['PCT_day_change_pre1'] < 1.3
            and (regression_data['PCT_day_change_pre2'] < 1.3 
            ...
            if(-2.95 > regression_data['PCT_day_change']
                ...
                add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL:HEAVYDOWNTRENDMARKET:HighDowntrend')
            elif(regression_data['PCT_day_change_pre1'] < 0
                or regression_data['PCT_day_change_pre2'] < 0
                or regression_data['PCT_day_change_pre3'] < 0
                ):
                add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL:HEAVYDOWNTRENDMARKET:HighDowntrend-(DOWNTREND-GLOBALDOWN)')
        ...
        if( -8 < regression_data['PCT_day_change'] < -4 and -10 < regression_data['PCT_change'] < -3
            ...
            if(regression_data['PCT_day_change_pre1'] > 0):
                add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL(9:30):HighDowntrend-2')
            else:
                add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL:HighDowntrend-2')
    ...
    if(-2.5 > regression_data['PCT_day_change'] > -5 and -2.5 > regression_data['PCT_change'] > -5
        ...
        add_in_csv(regression_data, regressionResult, ws, None, None, 'CommonHL:DownTrendLast2Day-GlobalDown-GlobalFutureDown-NiftyDown')
...
def is_reg_buyorsell_defined_filter(regression_data, regressionResult, high_or_low, ws, filter_avg, filter_count, filter_pct, risky):
    flag = False
    ...
    if(is_reg_buy_pct_filter(regression_data)
        ...
        and (('sell' not in reg_data_filter and 'Sell' not in reg_data_filter) 
             or (regression_data[filter_avg] > 2 and regression_data[filter_count] > 3 and regression_data[filter_pct] == 100)
             )
        ):
    ...
    if(is_reg_sell_pct_filter(regression_data)
        ...
        and (('buy' not in reg_data_filter and 'Buy' not in reg_data_filter) 
             or (regression_data[filter_avg] < -2 and regression_data[filter_count] > 3 and regression_data[filter_pct] == -100)
            )
        ):
        if(regression_data[filter_avg] < -2 and regression_data[filter_count] >= 2 and regression_data[filter_pct] < -70
...
def is_filter_all_accuracy(regression_data, regression_high, regression_low, regressionResult, high_or_low, ws):
    ...
    if((abs(regression_data['filter_345_avg']) > 2 and (regression_data['filter_345_count'] >= 3 or abs(regression_data['filter_345_pct']) == 100)) 
        or regression_data['filter_345_avg'] > 5
        ):
        ...
        if(flag):
            superflag = True   
    if((abs(regression_data['filter_avg']) > 2 and (regression_data['filter_count'] >= 3 or abs(regression_data['filter_pct']) == 100)) 
        or regression_data['filter_avg'] > 5
        ):
    ...
    if(flag):
        superflag = True 
    flag = is_reg_buyorsell_defined_filter(regression_data, regressionResult, high_or_low, ws, 'filter_all_avg', 'filter_all_count', 'filter_all_pct', True)    
    if(flag):
        superflag = True 
    ...
    if(abs(regression_data[filter_avg]) >= 0.5
        ...
        if(regression_data[filter_count] >= 2
            ...
            if((('sell' in reg_data_filter or 'Sell' in reg_data_filter) and (1 < regression_data[filter_avg] and  regression_data[filter_count] <= 3))
                ...
                and ('Buy-Risky' not in regression_data['filter2'] or regression_data['PCT_day_change_pre1'] < -1)
                and 'MLSell' not in regression_data['filter']
                ...
                and ('Sell-Risky' not in regression_data['filter2'] or regression_data['PCT_day_change_pre1'] > 1)
                and 'MLBuy' not in regression_data['filter'] 
                and 'STRONG-' not in regression_data['filter1']
                ):
                add_in_csv(regression_data, regressionResult, ws, None, None, None, 'WEAK-Sell-(LT-2)')
                flag = True
            elif(((regression_data[filter_count] >= 3 and regression_data[filter_pct] >= 79) 
                  or (regression_data[filter_count] >= 2 and 'Buy-SUPER' in regression_data['filter2'])
                  #or ('Buy-SUPER,' in regression_data['filter2'])
                  )
                and ('Buy-Risky' not in regression_data['filter2'] or regression_data['PCT_day_change_pre1'] < -1)
                and ('Sell-Any' not in regression_data['filter2'])
                and regression_data[filter_avg] > 1.5
                ...
                and 'MLSell' not in regression_data['filter']  
                and ('MLBuy' in regression_data['filter'] or 'Buy-SUPER' in regression_data['filter2'])
                and is_any_sell_from_all_filter(regression_data) == False
                ):
                add_in_csv(regression_data, regressionResult, ws, None, None, None, 'WEAK-Buy')
                flag = True
            elif(((regression_data[filter_count] >= 3 and regression_data[filter_pct] <= -79) 
                  or (regression_data[filter_count] >= 2 and 'Sell-SUPER' in regression_data['filter2'])
                  #or ('Sell-SUPER,' in regression_data['filter2'])
                  )
                and ('Sell-Risky' not in regression_data['filter2'] or regression_data['PCT_day_change_pre1'] > 1)
                and ('Buy-Any' not in regression_data['filter2'])
                and regression_data[filter_avg] < -1.5 
                ...
                and 'MLBuy' not in regression_data['filter']
                and ('MLSell' in regression_data['filter'] or 'Sell-SUPER' in regression_data['filter2']) 
                ...
                add_in_csv(regression_data, regressionResult, ws, None, None, None, 'RISKY-WEAK-Sell-(LT-2)')
                flag = True
            elif(((regression_data[filter_count] >= 3 and regression_data[filter_pct] >= 79) 
                  or (regression_data[filter_count] >= 2 and 'Buy-SUPER' in regression_data['filter2'])
                  #or ('Buy-SUPER,' in regression_data['filter2'])
                ...
                add_in_csv(regression_data, regressionResult, ws, None, None, None, 'RISKY-WEAK-Buy')
                flag = True
            elif(((regression_data[filter_count] >= 3 and regression_data[filter_pct] <= -79) 
                  or (regression_data[filter_count] >= 2 and 'Sell-SUPER' in regression_data['filter2'])
                  #or ('Sell-SUPER,' in regression_data['filter2'])
                ...
                add_in_csv(regression_data, regressionResult, ws, None, None, None, 'RISKY-WEAK-Sell')
                flag = True
        ...
        if('STRONG' not in regression_data['filter1']
            ...
            if(regression_data[filter_avg] >= 5
                ...
                add_in_csv(regression_data, regressionResult, ws, None, None, None, '**DUPER-Buy')
                flag = True
        ...
        if(("MLSell" not in regression_data['filter']) 
            and ((regression_data[filter_avg] >= 1.5 and (regression_data[filter_pct] >= 80 or regression_data[filter_pct] == 0))
                or (regression_data[filter_avg] >= 2 and regression_data[filter_pct] >= 70)
                )
            ):
            flag = True
