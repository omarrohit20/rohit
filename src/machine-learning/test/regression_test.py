...
def regression_ta_data(scrip):
    ...
    try:
        ...
        df['close_post5'] = df['close'].shift(-5)
        df['close_post10'] = df['close'].shift(-10)
        df['close_post20'] = df['close'].shift(-20)
        df['close_post40'] = df['close'].shift(-40)
        df['close_post60'] = df['close'].shift(-60)
        df['close_post5_change'] = (((df['close_post5'] - df['close']) / df['close']) * 100)
        df['close_post10_change'] = (((df['close_post10'] - df['close']) / df['close']) * 100)
        df['close_post20_change'] = (((df['close_post20'] - df['close']) / df['close']) * 100)
        df['close_post40_change'] = (((df['close_post40'] - df['close']) / df['close']) * 100)
        df['close_post60_change'] = (((df['close_post60'] - df['close']) / df['close']) * 100)
        df = df.round(2)
        
        df.dropna(subset=['Act_PCT_change'], inplace=True)
        df.dropna(subset=['close_post60_change'], inplace=True)
