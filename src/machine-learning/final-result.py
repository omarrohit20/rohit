...
def result_data(scrip):
    ...
    if(classification_data['kNeighboursValue'] > 0 and regression_data['kNeighboursValue'] > .5 and regression_data['forecast_day_PCT7_change'] < -1 and regression_data['forecast_day_PCT10_change'] < -1 and 5 > regression_data['PCT_day_change'] > -.5 and 5 > regression_data['forecast_day_PCT_change'] > 0):
        ws_buyFinal.append(regressionResult) 
        ws_buyFinal.append(classificationResult) 
    elif(classification_data['kNeighboursValue'] > 0 and regression_data['kNeighboursValue'] > .5 and 5 > regression_data['PCT_day_change'] > -.5):
