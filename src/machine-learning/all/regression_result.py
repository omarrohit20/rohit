...
def result_data(scrip):
    ...
    if(regression_data is not None and classification_data is not None
        ...
        buyIndiaAvg = buy_pattern_from_history(classification_data, regressionResult, ws_buyPattern2)
        if buy_all_rule(classification_data, regressionResult, buyIndiaAvg, ws_buyAll):
            buy_year_high(classification_data, regressionResult, ws_buyYearHigh)
            buy_year_low(classification_data, regressionResult, ws_buyYearLow, ws_buyYearLow1)
            buy_up_trend(classification_data, regressionResult, ws_buyUpTrend)
            buy_down_trend(classification_data, regressionResult, ws_buyDownTrend)
            buy_final(classification_data, regressionResult, ws_buyFinal, ws_buyFinal1)
            buy_high_indicators(classification_data, regressionResult, ws_buyHighIndicators)
            buy_pattern(classification_data, regressionResult, ws_buyPattern, ws_buyPattern1)
    ...
    if(regression_data is not None and classification_data is not None
        ...
        regressionResult = get_regressionResult(classification_data, scrip, db)
        sellIndiaAvg = sell_pattern_from_history(classification_data, regressionResult, ws_sellPattern2)
        if sell_all_rule(classification_data, regressionResult, sellIndiaAvg, ws_sellAll):
            sell_year_high(classification_data, regressionResult, ws_sellYearHigh, ws_sellYearHigh1)
            sell_year_low(classification_data, regressionResult, ws_sellYearLow)
            sell_up_trend(classification_data, regressionResult, ws_sellUpTrend)
            sell_down_trend(classification_data, regressionResult, ws_sellDownTrend)
            sell_final(classification_data, regressionResult, ws_sellFinal, ws_sellFinal1)
            sell_high_indicators(classification_data, regressionResult, ws_sellHighIndicators)
            sell_pattern(classification_data, regressionResult, ws_sellPattern, ws_sellPattern1)      
