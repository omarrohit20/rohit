    ...
    if(classification_data is not None):
        ...
        if(((classification_data['mlpValue'] >= 1 and classification_data['kNeighboursValue'] >= 0) or (classification_data['mlpValue'] >= 1 and classification_data['kNeighboursValue'] >= 1))
           and 'P@[' not in str(classification_data['sellIndia'])):
            ...
            if(-5 < classification_data['yearHighChange'] < -1 and classification_data['forecast_day_PCT2_change'] <= 5):
                ws_buyYearHigh.append(classificationResult)
            elif(-20 < classification_data['yearHighChange'] < -1 and classification_data['forecast_day_PCT2_change'] <= 5 and 0 < classification_data['PCT_day_change'] < 3 and (score == 'up'  or classification_data['forecast_day_PCT_change'] > 0)):
                ws_buyYearHigh.append(classificationResult)
                
            if((1 < classification_data['yearLowChange'] < 10 and 0 < classification_data['PCT_day_change'] < 3 and score == 'up')
                and (classification_data['forecast_day_PCT10_change'] <= -5 or classification_data['forecast_day_PCT5_change'] > 5)):
    ...
    if(classification_data is not None):
        ...
        if(((classification_data['mlpValue'] <= -1 and classification_data['kNeighboursValue'] <= 0) or (classification_data['mlpValue'] <= -1 and classification_data['kNeighboursValue'] <= -1))
           and 'P@[' not in str(classification_data['buyIndia'])):
            ...
            if(-10 < classification_data['yearHighChange'] < -2 and -2 < classification_data['PCT_day_change'] < 0.8 
                and classification_data['forecast_day_PCT5_change'] > 0 and (score == 'down'  or classification_data['forecast_day_PCT_change'] < 0)):
